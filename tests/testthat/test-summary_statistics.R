...
test_that("summarise_events_averted returns formatted output with median and quantiles", {
  input <- tibble(
    model = c("A", "A", "A", "B", "B"),
    run = c(1, 2, 3, 1, 2),
  ...
  result <- summarise_events_averted(input)

  # Check structure
  expect_s3_class(result, "tbl_df")
  expect_named(result, c("model", "m", "lc", "uc", "events_averted"))
  ...
  expect_true(all(grepl("95% UI", result$events_averted)))
})

test_that("summarise_events_averted works with different digits", {
  input <- tibble(
    model = rep("Z", 3),
    run = 1:3,
  ...
  result <- summarise_events_averted(input, digits = 3)
  expect_match(result$events_averted, "^[0-9]{2}.*\\(95% UI:")
})
